...
let playerFacingRight = true; // true = direita, false = esquerda
...
function updateHUD() {
    ...
    // Mostra quantos capangas ainda estão vivos (se ainda não mudou de fase)
    if (!cenarioMudado) {
        const capangasVivos = enemies.filter(enemy => enemy.alive && !enemy.isBoss).length;
        if (capangasVivos > 0) {
            esmeraldasHUD.innerHTML += ` | 👹 ${capangasVivos}`;
        }
    }
}
...
function movePlayer() {
    if (playerDead || jogoPausado) return;
    ...
    if (keys.a || keys.ArrowLeft) {
        playerX -= playerSpeed;
        moved = true;
        // Jogador andando para a esquerda
    ...
    if (keys.d || keys.ArrowRight) {
        playerX += playerSpeed;
        moved = true;
        // Jogador andando para a direita
        if (!playerFacingRight) {
    ...
    if (playerX >= POSICAO_FINAL && !cenarioMudado) {
        // Verifica se todos os capangas foram mortos
        const capangasVivos = enemies.filter(enemy => enemy.alive && !enemy.isBoss);
        if (capangasVivos.length === 0) {
            mudarFundo();
        } else {
            // Impede o jogador de passar e mostra mensagem
            playerX = POSICAO_FINAL - 10; // Empurra o jogador para trás
            player.style.left = playerX + 'px';
            mostrarMensagemEspecial("⚠️ Você deve derrotar todos os ogros capangas primeiro! ⚠️");
        }
    }
}
...
function moveEnemies() {
    ...
    enemies.forEach(enemy => {
        ...
        // Sistema de rotação: ogro vira de frente para o jogador
        const dx2 = playerX - enemy.x;
        if (dx2 > 0) {
            // Jogador está à direita do ogro - ogro olha para a direita (invertido)
            img.style.transform = "scaleX(-1)";
        } else if (dx2 < 0) {
            // Jogador está à esquerda do ogro - ogro olha para a esquerda (normal)
            img.style.transform = "scaleX(1)";
        }
    });
}
...
function attackPlayer(enemy) {
    ...
    if (Math.abs(playerX - enemy.x) < 40) {
        ...
        const img = enemy.element.querySelector("img");
        const currentTransform = img.style.transform; // Preserva a rotação atual
        img.src = enemy.isBoss ? ogroChefeAttackSprite : ogroCapangaAttackSprite;
        img.style.transform = currentTransform; // Mantém a rotação durante o ataque

        setTimeout(() => {
            if (enemy.alive) {
                img.src = enemy.isBoss ? ogroChefeWalkSprite : "../design-jogo/ogrocapangaandando.gif";
                img.style.transform = currentTransform; // Mantém a rotação após o ataque
            }
        }, 500);
...
function attackEnemies() {
    ...
    console.log("attackEnemies chamada! Inimigos vivos:", enemies.filter(e => e.alive).length);

    enemies.forEach(enemy => {
        if (!enemy.alive) return;
        const distancia = Math.abs(playerX - enemy.x);
        console.log("Verificando inimigo - distância:", distancia, "vida:", enemy.health);
        
        if (distancia < 60) {
            console.log("Inimigo dentro do alcance! Causando dano...");
            enemy.health -= enemy.isBoss ? 1 : 2;
            console.log("Vida do inimigo após dano:", enemy.health);
            
            if (enemy.health <= 0) {
                console.log("INIMIGO MORREU! Iniciando drop...");
                enemy.alive = false;
                enemy.element.style.display = "none";
                ...
                if (enemy.isBoss) {
                    // Chefe dropa muitas esmeraldas (15 esmeraldas)
                    console.log("🎉 CHEFE DERROTADO! 🎉 Dropando tesouro épico!");
                    mostrarMensagemEspecial("🏆 CHEFE DERROTADO! 🏆 Tesouro épico!");
                    
                    for (let i = 0; i < 15; i++) {
                        setTimeout(() => {
                            console.log("Dropando esmeralda do chefe", i + 1, "de 15");
                            droparGema(enemy);
                        }, i * 150); // Intervalo menor para um efeito mais dramático
                    }
                } else {
...
function droparGema(inimigo) {
    ...
    console.log("Esmeralda adicionada ao container");
...
document.addEventListener('keydown', e => {
    if (e.key === ' ' && !attacking && !playerDead && !jogoPausado) {
        attacking = true;
        const currentTransform = sprite.style.transform; // Preserva a rotação atual
        sprite.src = attackSpritePath;
        sprite.style.transform = currentTransform; // Mantém a rotação durante o ataque
        attackEnemies();
        setTimeout(() => {
            attacking = false;
            if (!playerDead) {
                sprite.src = walkSpritePath;
                sprite.style.transform = currentTransform; // Restaura a rotação após o ataque
            }
        }, 400);
...
document.addEventListener('keyup', e => {
    ...
    if (!Object.values(keys).some(v => v) && !attacking && !playerDead && !jogoPausado) {
        sprite.src = walkSpritePath;
        // Preserva a rotação quando para de andar
        sprite.style.transform = playerFacingRight ? "scaleX(1)" : "scaleX(-1)";
    }
});
...
function testarEsmeralda() {
    ...
    // Fallback caso a imagem não carregue
    gema.onerror = function() {
        console.log("Erro ao carregar imagem da esmeralda de teste, usando fallback");
        gema.style.backgroundColor = "#00ff00";
        gema.style.borderRadius = "50%";
        gema.style.border = "2px solid #ffffff";
    };
    ...
    gameContainer.appendChild(gema);
    console.log("Esmeralda de teste adicionada ao container");
    
    // Animação de queda melhorada
    let posY = 100;
    let velocidadeY = 0;
    const gravidade = 0.8;
    const chão = 522; // 600 - 50 - 28
    ...
    function animarQueda() {
        ...
        if (posY >= chão) {
            ...
            // Debug: mostra a posição atual da gema
            if (Math.floor(posY) % 50 === 0) {
                console.log("Gema de teste caindo - posY:", posY, "velocidadeY:", velocidadeY);
            }
    ...
    console.log("Iniciando animação de teste da esmeralda");
    requestAnimationFrame(animarQueda);
}

// Função para forçar drop de esmeralda (teste)
function forcarDropEsmeralda() {
    console.log("=== FORÇANDO DROP DE ESMERALDA ===");
    ...
    // Simula um inimigo morto
    const inimigoSimulado = {
        element: { getBoundingClientRect: () => ({ x: 400, y: 200 }) },
        isBoss: false
    };
    
    console.log("Chamando droparGema diretamente...");
    droparGema(inimigoSimulado);
}

// Função de teste para simular drop do chefe (pode ser chamada no console)
function testarDropChefe() {
    console.log("Testando drop épico do chefe...");
    const gameContainer = document.querySelector('.game-container');
    
    // Simula um inimigo chefe
    const chefeSimulado = {
        element: { getBoundingClientRect: () => ({ x: 400, y: 200 }) },
        isBoss: true
    };
    ...
    // Dropa 15 esmeraldas como o chefe real
    for (let i = 0; i < 15; i++) {
        setTimeout(() => {
            console.log("Dropando esmeralda do chefe", i + 1, "de 15");
            droparGema(chefeSimulado);
        }, i * 150);
    }
}
